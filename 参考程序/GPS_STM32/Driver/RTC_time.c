...
void Setup_RTC(void)
{
    ...
    if(BKP_ReadBackupRegister(BKP_DR1)!=0xA5A5)
    {
		...
		//设置年
		printp("\r\n\r\nSet year: ");
		now.tm_year = StringTODec();
		while( (now.tm_year<1990)||(now.tm_year>2200) )
		{
			printp("\r\nInput error!\r\nSet year: ");
			now.tm_year = StringTODec();
		}
		...
		//设置月
		printp("\r\nSet mon : ");
		now.tm_mon = StringTODec()-1;//转化成0-11月
		while(now.tm_mon>11)
		{
			printp("\r\nInput error!\r\nSet mon : ");
			now.tm_mon = StringTODec()-1;//转化成0-11月
		...
		//设置天
		printp("\r\nSet day : ");
        now.tm_mday = StringTODec();
		while(now.tm_mday>31)
		{
			printp("\r\nInput error!\r\nSet day : ");
			now.tm_mday = StringTODec();
		}
		...
		//设置小时
		printp("\r\nSet hour: ");
        now.tm_hour = StringTODec();
		while(now.tm_hour>23)
		{
			printp("\r\nInput error!\r\nSet hour: ");
			now.tm_hour = StringTODec();
		}
